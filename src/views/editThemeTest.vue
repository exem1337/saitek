<template>
  <div class="container">
    <h3 class="grey-text text-darken-2 headText z-depth-3">
      Редактирование теста: тема 1 сложность
    </h3>
    <div class="card horizontal z-depth-3 quesitons-wrapper">
      <test-edit-question
        v-for="(question, key) in testQuestions"
        :key="key"
        :question="question"
        :needSave="needSave"
        class="test-question"
    ...
    <a href="#bottom" class="btn blue fixed-right-bottom">Добавить вопрос</a>
    <div id="bottom"></div>
  </div>
</template>

<script>
import testEditQuestion from "../components/testEditQuestion.vue";
import { ref } from 'vue'
export default {
  components: { testEditQuestion },
  setup() {
    const testQuestions = [
      {
        Header:
