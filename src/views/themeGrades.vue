<template>
  <div class="container">
    <h3 class="grey-text text-darken-2 headText z-depth-3">
      Оценки студентов по теме {{ course.courseThemeName }}
    </h3>
    <ul class="collection z-depth-3 white" style="width: 100%; margin-top: 0">
      <li class="white" style="margin: 15px">
        <button
          @click="sortByGroup"
          class="waves-effect waves-light btn"
          style="background-color: #2f79ee"
        >
          <i class="material-icons left">sort</i>Отсортировать по группам
        </button>
      </li>
      <li
        v-for="student in course.students"
        :key="student.studentID"
        class="collection-item avatar userCard"
      >
        <img
        ...
        <span class="title"
          >{{ student.studentName }}
          {{ student.studentPatronim }} {{ student.studentLastName }}</span
        >
        <p>
          Группа {{ student.studentGroup }} <br />
        ...
        <a
          ...
          style="color: #2f79ee"
          ><i class="material-icons" style="color: #2f79ee">grade</i>Оценка:
