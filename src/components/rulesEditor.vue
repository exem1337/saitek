<template>
  <div class="card horizontal rulesContainer z-depth-3">
    ...
    <ul class="rulesCard">
      <li
        @click="editShow"
        class="rule z-depth-2 waves-effect"
        style="padding: 15px;"
        v-for="rule in rules"
        :key="rule.key"
      >
        {{ rule.text }}<br />
        <p class="flow-text rule-value" style="margin: 0;">{{ rule.value }}</p>
    ...
    const rules = [
      {
        ...
        text: "Правило 1",
        value:
          "Если УУД НИЗКИЙ И В НИЗКИЙ И У НИЗКИЙ И СКС НИЗКИЙ И СУ НИЗКИЙ И О НИЗКИЙ ТОГДА НОВИЧОК",
      },
      {
        key: 1,
        text: "Правило 2",
        value:
          "Если УУД ВЫСОКИЙ И В НИЗКИЙ И У НИЗКИЙ И СКС СРЕДНИЙ И СУ НИЗКИЙ И О НИЗКИЙ ТОГДА НОВИЧОК",
      },
      {
        key: 2,
        text: "Правило 3",
        value:
          "Если УУД НИЗКИЙ И В ВЫСОКИЙ И У НИЗКИЙ И СКС НИЗКИЙ И СУ НИЗКИЙ И О НИЗКИЙ ТОГДА НОВИЧОК",
      },
      {
        key: 3,
        text: "Правило 4",
